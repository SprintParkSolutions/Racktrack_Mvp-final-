...
def get_me(token):
    resp = SESSION.get(f"{GRAPH_BASE}/me", headers={"Authorization": f"Bearer {token}"})
...
def upload_and_share_file(token, file_path, recipient_email):
    ...
    # An organisation-scoped share link already gives every tenant member access,
    # so we skip the extra per-user /invite round-trip (it was slow and redundant).
...
def main():
    ...
    try:
        ...
        # Chat creation and file upload are independent — run them concurrently
        # instead of back-to-back to roughly halve the round-trip latency.
        with ThreadPoolExecutor(max_workers=2) as pool:
            chat_future = pool.submit(get_or_create_chat, token, my_id, args.email)
            upload_future = pool.submit(upload_and_share_file, token, args.file, args.email)
            chat_id = chat_future.result()
