...
def annotate_units_only(img, units):
    ...
    for unit in units:
        ...
        cv2.putText(
            out, unit["label"], (x1, y1 - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.8, (0, 255, 0), 2
        )
    return out
...
def annotate_devices_only(img, devices):
    ...
    for idx, device in enumerate(devices, start=1):
        ...
        color = DEVICE_NAME_COLOR.get(
            device["class_name"], DEVICE_CLASS_COLOR.get(cls_id, (255, 0, 0))
        )
        units = device.get("units") or []
        units_label = ",".join(units) if units else "unknown"
        label = f"{idx}:{device['class_name']} [{units_label}]"
        cv2.rectangle(out, (x1, y1), (x2, y2), color, 3)
        cv2.putText(out, label, (x1, y2 + 20), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 1)
    return out
