...
# Per-vendor switch lineups. When the scan/OCR reads a make (even without a
...
def synthesize_switch(rack_id: str, name: str, u: int, scan_dev: dict) -> dict:
    ...
    make_hint = scan_dev.get("make") or scan_dev.get("ocr_make")
    model_number = real_model or _switch_model(rack_id, name, make_hint)
    # If the model is real, it's no longer a synthetic field.
    synth_fields = ["serial_number", "mgmt_ip", "mac", "os", "port_prefix"]
    if not real_model:
        synth_fields.insert(0, "model_number")
    return {
        "_synthetic": True,
...
def synthesize_patch_panel(rack_id: str, name: str, scan_dev: dict) -> dict:
    main = int(scan_dev.get("port_count") or 0)
    real_model = _real_model(scan_dev)
    model_number = real_model or _pick(rack_id, name, PATCH_PANEL_MODELS, "panel_model")
    synth_fields = ["serial_number"] if real_model else ["model_number", "serial_number"]
    return {
        "_synthetic": True,
        "_synthetic_fields": synth_fields,
        "model_number": model_number,
...
def apply_ocr_to_switch(synth_dev: dict, ocr: dict, rack_id: str = "", name: str = "") -> dict:
