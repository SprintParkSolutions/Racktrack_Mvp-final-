...
from synth import SYNTH_TAG, build_inventory, load_override, load_port_detail, merge_port_detail
...
def derive_wiring(scan: dict, inv: dict) -> list[tuple]:
    ...
    if (
        server
        and "SW-U10" in switches
        and "PP-U08" in panels
        ...
        cables.append(
            (
                "SW-U10",
                port_label("SW-U10", sp),
                "PP-U08",
                port_label("PP-U08", pp),
                "C-0142",
                "Cat6a",
                "2m",
            )
    ...
    if (
        server
        and "SW-U02" in switches
        and "PP-U06" in panels
        ...
        cables.append(
            (
                "SW-U02",
                port_label("SW-U02", sp),
                "PP-U06",
                port_label("PP-U06", pp),
                "C-0144",
                "Cat6a",
                "2m",
            )
...
def write_topology_snapshot(scan: dict, inv: dict, cables: list[tuple], output_dir: str) -> dict:
    ...
    for sw_name, details in switches.items():
        ...
        devices.append(
            {
                "name": sw_name,
                "class": "router" if sw_name.startswith("RTR-") else "switch",
                "u_position": u,
                "u_size": 1,
                ...
                "ports": ports,
                "summary": f"{total} port{'s' if total != 1 else ''}"
                if total
                    ...
                    "disks": [
                        {
                            ...
                            "partitions": [
                                {"name": p["name"], "size_bytes": p.get("size_bytes")}
    ...
    snapshot = {
        ...
        "rackName": rack_name,
        "u_size": u_size,
        "generated_at": dt.datetime.now(dt.timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ"),  # noqa: UP017 (3.10)
...
def write_synthesis_log(inv: dict, output_dir: str) -> dict:
    ...
    for name, sw in inv["switches"].items():
        if sw.get("_synthetic"):
            log["synthetic"].append(
                {"name": name, "kind": "switch", "fields": sw.get("_synthetic_fields") or []}
            )
    for name, pp in inv["panels"].items():
        if pp.get("_synthetic"):
            log["synthetic"].append(
                {"name": name, "kind": "patch_panel", "fields": pp.get("_synthetic_fields") or []}
            )
    if inv["server"] and inv["server"].get("_synthetic"):
        log["synthetic"].append(
            {
                "name": inv["server"]["name"],
                "kind": "server",
...
def main() -> int:
    ...
    print(
        f"  synthetic CIs:   {n_synth} ({'override file present' if override else 'no override file — fully synthesized'})"
    )
    return 0
